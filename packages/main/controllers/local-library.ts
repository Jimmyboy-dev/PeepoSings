...
class LocalIpcCtrl {
  ...
  async setLocalFolders(event: IpcMessageEvent, directories: string[]) {
    const localFolders = await Promise.all(directories.map((folder) => this.localLibraryDb.addFolder(this.normalizeFolderPath(folder))))

    const cache = await this.localLibrary.scanFoldersAndGetMeta(localFolders, (scanProgress: any, scanTotal: any) => {
      this.window.send(IpcEvents.LOCAL_FILES_PROGRESS, { scanProgress, scanTotal })
    })

    this.window.send(
      IpcEvents.LOCAL_FILES,
  ...
  async removeLocalFolder(event: IpcMessageEvent, localFolder: string) {
    const metas = await this.localLibraryDb.removeLocalFolder(this.normalizeFolderPath(localFolder))
  ...
  async onRefreshLocalFolders() {
    try {
      const folders = await this.localLibraryDb.getLocalFolders()
      const cache = await this.localLibrary.scanFoldersAndGetMeta(folders, (scanProgress: any, scanTotal: any) => {
        this.window.send(IpcEvents.LOCAL_FILES_PROGRESS, { scanProgress, scanTotal })
      })
