...
import type LocalFolder from './models/LocalFolder'
import AcousticId from '../acoustic-id'
...
class LocalLibrary {
   ...
   */
  private async formatMeta({ common, format }: IAudioMetadata, { file, folder }: { file: string; folder?: LocalFolder }): Promise<PeepoMeta> {
    return {
      uuid: v4(),
      path: file,
      folder,
      ...
      position: common.track.no ?? undefined,
      album: common.album,
  ...
  private async parseMeta(filesPath: Array<{ file: string; folder?: LocalFolder }>, onProgress?: ProgressHandler): Promise<PeepoMeta[]> {
  ...
  async getMetas(filesPath: string[], onProgress?: ProgressHandler): Promise<PeepoMeta[]> {
    return this.parseMeta(
      filesPath.map((file) => ({ file })),
      onProgress
    )
  }
   ...
   */
  fetchAcousticIdBatch(metas: PeepoMeta[], onProgress?: ProgressHandler) {
    let scanProgress = 0
  ...
  async scanFoldersAndGetMeta(directories: LocalFolder[], onProgress?: ProgressHandler): Promise<Partial<LocalTrack>[]> {
    const filesData = await Promise.all(
      _.flatMap(this.config.supportedFormats, (format) =>
        Promise.all(
    ...
    const files = filesData.flat().reduce<Array<{ file: string; folder: LocalFolder }>>((acc, item) => {
      return item.files
        .map((filePath) => ({
          file: filePath,
          folder: item.folder,
  ...
  async playStartupFile(filePath: string) {
    try {
      const metas = await this.getMetas([path.isAbsolute(filePath) ? filePath : path.resolve(process.cwd(), filePath)])
