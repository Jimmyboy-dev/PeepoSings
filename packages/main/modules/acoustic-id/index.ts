...
class AcousticId {
  // lastfm: LastFMTyped
  constructor(@inject(Config) private config: Config, @inject(Platform) private platform: Platform) {
    // this.lastfm = new LastFMTyped(config.acousticId.key, { apiSecret: config.acousticId.secret }) //insert key, secret, user agent, and whether to use https here
  }
