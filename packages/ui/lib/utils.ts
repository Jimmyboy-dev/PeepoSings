...
import type { Album, Track, TrackItem } from './types'
...
export const getTrackArtist = (track: Track) => (_.isString(track?.artist) ? track?.artist : track?.artist?.name)

export const getThumbnail = (albumOrTrack) => _.get(albumOrTrack, 'coverImage') || _.get(albumOrTrack, 'thumb') || _.get(albumOrTrack, 'thumbnail')
...
export const timestampToDateString = (timestamp: number, locale: string) =>
...
export const timestampToTimeString = (timestamp: number, locale: string) =>
  new Date(timestamp).toLocaleDateString(locale, {
    year: 'numeric',
    month: 'numeric',
    day: 'numeric',
