...
import type { StreamData, StreamQuery } from '../plugins/plugins.types'
...
function analyseUrlType(url: string) {
...
function handleYoutubeVideo(url: string) {
  return ytdl
    .getInfo(url)
    .then((info) => {
...
export async function liveStreamSearch(query: string) {
  ...
  return (searchResults.items as any[]).map((video: ytsr.Video) => {
    return {
