...
const penalizedWords = ['remix', 'full album', 'album']

const promotedWords = ['hq', 'high quality', 'official']

export class YoutubeHeuristics implements SearchHeuristics<Partial<Video>> {
  static createTitle = ({ artist, title }: { artist: string; title: string }) => `${artist} - ${title}`.toLowerCase()

  scoreTrack = ({ track, artist, title, duration }: ScoreTrackArgs<Partial<Video>>) => {
    ...
    const durationScore = track.duration && duration ? (1 - durationDelta / duration) * 100 : 0
    ...
    return mean([titleScore, durationScore, promotedWordsScore, penalizedWordsScore, liveVideoScore])
  }

  orderTracks = ({ tracks, artist, title, duration }: OrderTracksArgs<Partial<Video>>) => {
