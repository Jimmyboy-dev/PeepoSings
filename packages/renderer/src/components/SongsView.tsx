    ...
    <Box className="flex flex-col gap-1 bg-gray-800 p-2 rounded-lg w-full">
      <div className="flex flex-row px-4">
        ...
        <Checkbox
          ...
          label="Compact View"
          checked={compact}
          onChange={(e) => dispatch(toggleCompactSongView(e.currentTarget.checked))}
        />
      </div>
      ...
      <Group direction="column" className="w-full mb-24" spacing={compact ? 0 : 16}>
        ...
        {songs.length !== 0 &&
          songs.map((song, i) => (
            <SongView compact={compact} song={song} setCurSong={() => dispatch(setCurrentSong(i))} key={song.title} />
          ))}
      </Group>
    </Box>
  )
    ...
    <div
      className={`flex flex-row items-center w-full text-gray-100 ${
        compact
          ...
          : "cursor-pointer p-2 rounded-lg h-24 hover:bg-base-100 transition-colors hover:font-semibold"
      ...
      <img
        src={song.albumArt}
        className={`h-full object-cover ${compact ? "aspect-square rounded-full p-1" : "rounded-xl"}`}
      />
      <div
        className={`flex  ${
          compact ? "flex-row gap-2 items-center flex-nowrap ml-2" : "flex-col ml-4 flex-grow justify-center"
        }`}>
            ...
            {song.mood.map(
              (id, i) =>
                id && (
                  <Badge
                    key={`${song.id}-mood-${i}`}
                    color={chooseRandomArrayValueSeeded(
                      [
                        "cyan",
                        "blue",
                        "green",
