...
import { DownloadInfo, IpcEvents, VideoInfo } from '@peepo/core'
import { inject, injectable } from 'inversify'
import Window from './window'
import { Database } from './Database'
import YTDlpWrap, { YTDlpReadable } from 'yt-dlp-wrap/dist/index'
...
export class MusicManager {
  ...
  ytdl: YTDlpWrap
  musicPath: string
  currentDownload: YTDlpReadable | null = null
  dlInfo?: DownloadInfo
  ...
  public async getYoutubeVideoInfo(url: string) {
    if (!url || url.length < 1) return {} as VideoInfo
