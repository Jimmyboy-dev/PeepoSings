...
import { PeepoMeta, IpcEvents } from '@peepo/core/index.js'
...
class TrayMenu {
  ...
  constructor(@inject(Config) private config: Config, @inject(Platform) private platform: Platform, @inject(Window) private window: Window) {}

  init() {
    const icon = nativeImage.createFromPath(this.platform.isMac() ? this.config.macIcon : this.config.icon)
